...
def colorear(clase, regions ):
   ...
   if clase==regions['region_attributes']['clase']:
      ...
      if clase=='celular': R,V,A=  [128,0,0]
      elif clase=='plato': R,V,A=  [0,0,128]
      elif clase=='vaso': R,V,A=  [128,128,0]
      elif clase=='herramienta': R,V,A=  [128,0,128]
      elif clase=='conector': R,V,A=  [128,128,128]
      elif clase=='persona': R,V,A=  [192,0,0]
      elif clase=='garrafon': R,V,A=  [0,192,0]
      elif clase=='obstaculo': R,V,A=  [0,0,192]
      elif clase=='mesa':  R,V,A=  [0,128,0]
      else : R,V,A= 0,0,0
...
IMAGE_FOLDER = "./Imagenes/ImagenesR/"
...
for imgId in imgs:
    ...
    for i in range(len(regions)): colorear('garrafon', regions[i])  
    for i in range(len(regions)): colorear('persona', regions[i]) 
    for i in range(len(regions)): colorear('herramienta', regions[i])
    for i in range(len(regions)): colorear('obstaculo', regions[i])     
    ...
    for i in range(len(regions)): colorear('conector', regions[i])    
